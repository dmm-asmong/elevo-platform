...
```
사용자: 구글 캘린더 일일 브리핑 앱을 만들고 싶어요.
사용자: /office-hours

Claude: [6가지 핵심 질문으로 진짜 문제를 추출]
        "당신이 필요한 건 '일일 브리핑 앱'이 아니라 개인 비서 AI입니다."
        → 3가지 구현 방향 + 출시 우선순위 + 설계문서

사용자: /plan-ceo-review   → 범위 검토
사용자: /plan-eng-review   → 아키텍처 확정
        (Claude가 11개 파일, 2,400줄 코드 작성 ~8분)
사용자: /review             → 2개 버그 자동 수정
사용자: /qa https://staging.myapp.com → 브라우저 테스트 & 버그 수정
사용자: /ship               → 테스트 42→51개, PR 생성
```
